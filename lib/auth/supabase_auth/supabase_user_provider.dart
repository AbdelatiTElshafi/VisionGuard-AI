...
class VisionGuardAISupabaseUser extends BaseAuthUser {
  VisionGuardAISupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> visionGuardAISupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = VisionGuardAISupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
